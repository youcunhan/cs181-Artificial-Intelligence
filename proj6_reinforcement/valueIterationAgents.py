...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeActionFromValues(self, state):
        ...
        action = None
        max_val = -float("inf")
        for each in self.mdp.getPossibleActions(state):
          qValue = self.computeQValueFromValues(state, each)
          if qValue > max_val:
            max_val = qValue
            action = each
        return action
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            if not self.mdp.isTerminal(state):
                self.values[state] = self.eachState(state)
